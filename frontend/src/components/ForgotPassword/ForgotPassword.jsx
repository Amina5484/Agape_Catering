...
const ForgotPassword = () => {
    ...
    return (
        <div className={`min-h-screen flex items-center justify-center py-8 px-4 sm:px-6 lg:px-8 transition-colors duration-200 ${isDarkMode ? 'bg-gray-900 text-white' : 'bg-gray-50 text-gray-900'}`}>
                    ...
                    <div className="px-4 py-5 space-y-4">
                        <div className="flex items-center mb-3">
                            <button
                                ...
                                className={`p-2 rounded-full mr-2 transition-colors ${isDarkMode ? 'text-gray-300 hover:bg-gray-700' : 'text-gray-600 hover:bg-gray-100'}`}
                            >
                        ...
                        {step === 1 && (
                            <form onSubmit={handleEmailSubmit} className="space-y-3">
                                <div className="flex justify-center my-4">
                                    <div className={`p-2.5 rounded-full ${isDarkMode ? 'bg-indigo-900' : 'bg-indigo-100'}`}>
                                        <FiMail className={`h-5 w-5 ${isDarkMode ? 'text-indigo-300' : 'text-indigo-600'}`} />
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    <div className="relative">
                                        <input
                                            ...
                                            className={`appearance-none relative block w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:z-10 sm:text-sm transition-colors duration-200 ${isDarkMode
                        ...
                        {step === 2 && (
                            <form onSubmit={handleOtpVerify} className="space-y-3">
                                <div className="flex justify-center my-4">
                                    <div className={`p-2.5 rounded-full ${isDarkMode ? 'bg-indigo-900' : 'bg-indigo-100'}`}>
                                        <FiKey className={`h-5 w-5 ${isDarkMode ? 'text-indigo-300' : 'text-indigo-600'}`} />
                                    </div>
                                </div>
                                ...
                                <div className="flex items-center text-xs">
                                    <FiRefreshCw className={`mr-1 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`} size={12} />
                                    <span
                                        onClick={() => handleEmailSubmit({ preventDefault: () => { } })}
                                        className={`cursor-pointer ${isDarkMode ? 'text-indigo-400 hover:text-indigo-300' : 'text-indigo-600 hover:text-indigo-500'}`}
                                    >
                                        Resend OTP
                                    </span>
                                </div>
                        ...
                        {step === 3 && (
                            <form onSubmit={handlePasswordReset} className="space-y-3">
                                <div className="flex justify-center my-4">
                                    <div className={`p-2.5 rounded-full ${isDarkMode ? 'bg-indigo-900' : 'bg-indigo-100'}`}>
                                        <FiLock className={`h-5 w-5 ${isDarkMode ? 'text-indigo-300' : 'text-indigo-600'}`} />
                                    </div>
                                </div>

                                <div>
                                    <label htmlFor="new-password" className={`block text-sm font-medium mb-1 ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                                        New Password
                                    </label>
                                    <input
                                        id="new-password"
